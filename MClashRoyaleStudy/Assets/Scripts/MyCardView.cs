...
public class MyCardView : MonoBehaviour,IDragHandler,IPointerUpHandler,IPointerDownHandler
{
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        if (hitGround)
        {
            ...
            /*
             * 一个已经销毁的对象 启动协程不能成功, 需要用存在场景中的mono脚本来启动协程
            //再发牌
            StartCoroutine(CardManager.instance.PromoteFromDeck(index,.5f));
            //再创建一张牌
            StartCoroutine(CardManager.instance.CreateCardInPreviewArea(1f));
            */
